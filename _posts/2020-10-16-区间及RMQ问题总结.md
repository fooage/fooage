...
void Build(int left, int right, int pos) {
  if (left == right) {
    // The leaf node of the line segment tree is the value of the original array.
    tree[pos] = base[left];
  } else {
...
int Query(int left, int right, int curLeft, int curRight, int pos) {
  if (curRight < left || curLeft > right) {
    ...
    // Return the sum of the completely contained interval.
    return tree[pos];
  } else {
    // Push down the marked interval and divide it to continue searching.
    int mid = (curLeft + curRight) / 2;
    Push(curLeft, curRight, pos);
...
void Init() {
  ...
  nums = n / perSize;
  if (n > perSize * nums) {
    // If there are rest elements, they are divided into independent blocks.
    nums++;
  }
  // Divide the beginning and end of an interval.
  ...
  // Set the attribution of each interval element.
  for (int i = 1; i <= nums; i++) {
    for (int j = st[i]; j <= ed[i]; j++) {
      belong[j] = i;
    }
  }
  // Record the size of these blocks.
  for (int i = 1; i <= nums; i++) {
    size[i] = ed[i] - st[i] + 1;
  }
  // Initializes the interval sum of blocks.
  for (int i = 1; i <= nums; i++) {
    for (int j = st[i]; j <= ed[i]; j++) {
      block[i] += base[j];
    }
...
void Update(int left, int right, int val) {
  if (belong[left] == belong[right]) {
    ...
    // Change both ends, the complete block can be marked with lazy.
...
int Query(int left, int right) {
  int ans = 0;
  // Don't forget to validate the lazy tag during the query.
  if (belong[left] == belong[right]) {
    for (int i = left; i <= right; i++) {
